...
- `PYTHONPATH=src python3 -m overseer.cli inspect-packages` - inspect apt package update availability without installing, upgrading, or refreshing package indexes.
...
- `PYTHONPATH=src python3 -m overseer.cli discover-user-services --store state/overseer.sqlite3` - capture running systemd user services as managed service resources.
...
- `PYTHONPATH=src python3 -m overseer.cli active-policy-profile --store state/overseer.sqlite3` - show the store-local active policy profile or the best-practice fallback.
