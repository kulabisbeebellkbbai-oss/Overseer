...
`GET /storage/evidence` and `GET /storage/backup-operations` include Kira backup provider targets, provider classes, and readiness rows. `//MediaStore/Overseer` is the first planned local NAS remote backup target. Cloud object storage, full clone, and hosted failover rows are future work until services are available for testing. These rows do not mount, connect to, copy to, or mutate any remote storage.
