...
`POST /claims/cleanup-requests/execute` executes only an approved cleanup request after re-validating the cleanup candidate. It can mark an expired active-like claim expired or re-evaluate a stale queued claim; missing-evidence and blocked-claim actions remain non-executable until they are revised.
