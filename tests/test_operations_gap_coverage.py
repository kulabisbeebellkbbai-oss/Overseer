...
class OperationsGapCoverageTests(unittest.TestCase):
    ...
    def test_storage_evidence_finds_backup_markers_without_mutation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "state").mkdir()
            (root / "state" / "app.sqlite3").write_text("sqlite placeholder\n", encoding="utf-8")
        ...
        alert_kinds = {row["kind"] for row in payload["risk_alerts"]}
        self.assertIn("local_database", alert_kinds)
        self.assertIn("database_wal", alert_kinds)
